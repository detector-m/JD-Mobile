...
#import "DZCommodityInfoCell.h"
#import "DZCommoditySelectCell.h"
#import "DZLoginViewController.h"
#import "DZNavigationController.h"
#import "DZDetailsModel.h"

@interface DZDetailsViewController ()<UITableViewDataSource, UITableViewDelegate,DZCycleScrollViewDelegate,MBProgressHUDDelegate>
{
    MBProgressHUD *HUD;
    NSArray *_images;
    UILabel *_indexPage;
...
/** 数据模型*/
@property (nonatomic, strong) DZDetailsModel *modelToShow;

@end

@implementation DZDetailsViewController

- (DZDetailsModel *)modelToShow {
    if (!_modelToShow) {
        _modelToShow = [DZDetailsModel new];
        // 假数据
...
- (NSMutableArray *)dataArray {
    if (!_dataArray) {
        ...
        // 购买信息
        DZCommodityCellConfig *comInfo = [DZCommodityCellConfig cellConfigWithClassName:NSStringFromClass([DZCommodityInfoCell class]) title:@"购买信息" showInfoMethod:@selector(showInfo:) heightOfCell:kHeightCommodityInfo cellType:NO];
        
        DZCommodityCellConfig *comSelect = [DZCommodityCellConfig cellConfigWithClassName:@"CommoditySelectCell" title:@"已选" showInfoMethod:@selector(showInfo:) heightOfCell:80.0f cellType:YES];
        
        DZCommodityCellConfig *comAddress = [DZCommodityCellConfig cellConfigWithClassName:@"DZCommodityAddressCell" title:@"地址" showInfoMethod:@selector(showInfo:) heightOfCell:180.0f cellType:YES];
        
        DZCommodityCellConfig *comPack = [DZCommodityCellConfig cellConfigWithClassName:@"DZCommodityPackCell" title:@"包装" showInfoMethod:@selector(showInfo:) heightOfCell:50.0f cellType:YES];
        
        DZCommodityCellConfig *comPraise = [DZCommodityCellConfig cellConfigWithClassName:@"DZCommodityPraiseCell" title:@"评价" showInfoMethod:@selector(showInfo:) heightOfCell:80.0f cellType:YES];
        
        DZCommodityCellConfig *comSection = [DZCommodityCellConfig cellConfigWithClassName:@"DZCommoditySectionCell" title:@"专区" showInfoMethod:@selector(showInfo:) heightOfCell:110.0f cellType:YES];
        
        DZCommodityCellConfig *comComponent = [DZCommodityCellConfig cellConfigWithClassName:@"DZCommodityComponentCell" title:@"配件" showInfoMethod:@selector(showInfo:) heightOfCell:50.0f cellType:YES];
...
- (void)initView{
    ...
    [DZMasonyUtil equalSpacingView:@[focus,cart]
...
- (UIView *)addHeaderView{
    ...
    DZCycleScrollView *cycleScrollView = [DZCycleScrollView cycleScrollViewWithFrame:CGRectMake(0, 0, view.size.width, view.size.height) imagesGroup:_images];
    cycleScrollView.autoScroll = NO;
    cycleScrollView.infiniteLoop = NO;
    cycleScrollView.delegate = self;
    cycleScrollView.pageControlStyle = DZCycleScrollViewPageContolStyleNone;
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    // 拿到cellConfig
    DZCommodityCellConfig *cellConfig = self.dataArray[indexPath.section][indexPath.row];
    // 拿到对应cell并根据模型显示
...
- (void)cartClick{
    DZCartViewController *cart = [[DZCartViewController alloc]init];
