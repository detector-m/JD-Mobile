 ...
 */
- (void)initialization {
    self.dotViewClass           = [DZAnimatedDotView class];
...
- (void)touchesBegan:(NSSet *)touches withEvent:(UIEvent *)event {
    ...
    if (touch.view != self) {
        ...
        if ([self.delegate respondsToSelector:@selector(DZPageControl:didSelectPageAtIndex:)]) {
            [self.delegate DZPageControl:self didSelectPageAtIndex:index];
        }
    }
}
...
- (UIView *)generateDotView {
    ...
    if (self.dotViewClass) {
        ...
        if ([dotView isKindOfClass:[DZAnimatedDotView class]] && self.dotColor) {
            ((DZAnimatedDotView *)dotView).dotColor = self.dotColor;
        }
    } else {
...
- (void)changeActivity:(BOOL)active atIndex:(NSInteger)index {
    if (self.dotViewClass) {
        DZAbstractDotView *abstractDotView = (DZAbstractDotView *)[self.dots objectAtIndex:index];
